    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        ...
        htmlContent.append(   
            "<tr class='isi'>"+
                ...
                "<td valign='middle' bgcolor='#FFFAFA' align='center' width='60px'>Cara Bayar</td>"+    
            "</tr>"
        ); 
        LoadHTML.setText(
                "<html>"+
    ...
    private void prosesCari() {
        ...
        try {
            ...
            ps=koneksi.prepareStatement("select reg_periksa.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,poliklinik.nm_poli,dokter.nm_dokter,date_format(reg_periksa.tgl_registrasi,'%d/%m/%Y') as tgl_registrasi,reg_periksa.jam_reg,"+
                    //"date_format(mutasi_berkas.dikirim,'%H:%i:%s')as dikirim,"
                     "IFNULL((SELECT DATE_FORMAT(jam_rawat, '%H:%i:%s') FROM pemeriksaan_ralan WHERE pemeriksaan_ralan.no_rawat = reg_periksa.no_rawat AND pemeriksaan_ralan.nip <> reg_periksa.kd_dokter ORDER BY jam_rawat ASC LIMIT 1), '00:00:00') AS diterima,date_format(nota_jalan.jam,'%H:%i:%s')as jambilling,"+
                    //"IFNULL((SELECT DATE_FORMAT(jam_rawat, '%H:%i:%s') FROM pemeriksaan_ralan WHERE pemeriksaan_ralan.no_rawat = reg_periksa.no_rawat AND pemeriksaan_ralan.nip=reg_periksa.kd_dokter ORDER BY jam_rawat ASC LIMIT 1), '00:00:00') AS dilayanipoli,"+
                    ...
                    "ROUND((TIME_TO_SEC(IFNULL((SELECT TIME(jam_rawat) FROM pemeriksaan_ralan WHERE pemeriksaan_ralan.no_rawat = reg_periksa.no_rawat AND pemeriksaan_ralan.nip <> reg_periksa.kd_dokter ORDER BY tgl_perawatan, jam_rawat ASC LIMIT 1),'00:00:00')) - TIME_TO_SEC(reg_periksa.jam_reg))/60, 2) AS JRsdBP,"+
                    "round((TIME_TO_SEC(date_format(nota_jalan.jam,'%H:%i:%s'))-TIME_TO_SEC(reg_periksa.jam_reg))/60,2) as JRsdTB,"+
                    //"ROUND((TIME_TO_SEC(IFNULL((SELECT TIME(jam_rawat) FROM pemeriksaan_ralan WHERE pemeriksaan_ralan.no_rawat = reg_periksa.no_rawat AND pemeriksaan_ralan.nip <> reg_periksa.kd_dokter ORDER BY tgl_perawatan, jam_rawat ASC LIMIT 1),'00:00:00'))-TIME_TO_SEC(date_format(mutasi_berkas.dikirim,'%H:%i:%s')))/60,2) as SBsdBP,"+
                    //"round((TIME_TO_SEC(date_format(nota_jalan.jam,'%H:%i:%s'))-TIME_TO_SEC(date_format(mutasi_berkas.dikirim,'%H:%i:%s')))/60,2) as SBsdTB,"+
                    "ROUND((TIME_TO_SEC(nota_jalan.jam)-TIME_TO_SEC(IFNULL((SELECT TIME(jam_rawat) FROM pemeriksaan_ralan WHERE pemeriksaan_ralan.no_rawat = reg_periksa.no_rawat AND pemeriksaan_ralan.nip <> reg_periksa.kd_dokter ORDER BY tgl_perawatan, jam_rawat ASC LIMIT 1), '00:00:00')))/60, 2) AS BPsdTB,penjab.png_jawab  "+
                    "from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis inner join poliklinik on reg_periksa.kd_poli=poliklinik.kd_poli inner join dokter on reg_periksa.kd_dokter=dokter.kd_dokter inner join penjab on reg_periksa.kd_pj=penjab.kd_pj "+
            ...
            try {
                ...
                rs=ps.executeQuery();
                i=1;
                ttlJRsdBP=0;ttlJRsdDP=0;ttlJRsdRDk=0;ttlJRsdRDv=0;ttlJRsdTB=0;ttlSBsdBP=0;ttlSBsdDP=0;ttlSBsdRDk=0;ttlSBsdRDv=0;ttlSBsdTB=0;
                ...
                while(rs.next()){
                    dilayanipoli=Sequel.cariIsi("select date_format(pemeriksaan_ralan.jam_rawat,'%H:%i:%s') from pemeriksaan_ralan where pemeriksaan_ralan.no_rawat=? order by pemeriksaan_ralan.tgl_perawatan,pemeriksaan_ralan.jam_rawat desc limit 1",rs.getString("no_rawat"));
                    //dilayanipoli=Sequel.cariIsi("select ifnull((select date_format(pemeriksaan_ralan.jam_rawat,'%H:%i:%s') from pemeriksaan_ralan where pemeriksaan_ralan.no_rawat=? and pemeriksaan_ralan.nip=select kd_dokter from reg_periksa where no_rawat=? order by pemeriksaan_ralan.tgl_perawatan, pemeriksaan_ralan.jam_rawat asc limit 1),'00:00:00')", rs.getString("no_rawat"));
                ...
                if(i>1){
                    htmlContent.append(
                        "<tr class='isi'>"+
                            "<td valign='middle'></td>"+
                            "<td valign='middle' align='left' colspan='11'>Rata-rata Pelayanan Pasien : </td>"+
