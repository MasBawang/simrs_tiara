...
public final class DlgRanapPerRuang extends javax.swing.JDialog {
    ...
    private int i=0,jmlpasien=0,ttljmlpasien=0,jmlhari=0,ttljmlhari=0,jmlpindah=0,ttljmlpindah=0;
    ...
    public DlgRanapPerRuang(java.awt.Frame parent, boolean modal) {
        ...
        setSize(885,674);
        tabMode=new DefaultTableModel(null,new String[]{"No.","Ruangan","Jumlah Pasien","Hari Perawatan","Pindah"}){
        ...
        tabMode2=new DefaultTableModel(null,new String[]{"No.","Ruangan","Jumlah Pasien","Hari Perawatan","Pindah"}){
              @Override public boolean isCellEditable(int rowIndex, int colIndex){return false;}
        };
        ...
        try{
            ...
            try {
                ...
                while(rs.next()){
                    ...
                    // Hitung jumlah pasien pindah
                ps2=koneksi.prepareStatement(
                    "select count(distinct reg_periksa.no_rawat) as jmlpindah "+
                     "from reg_periksa "+
                     "inner join kamar_inap on reg_periksa.no_rawat=kamar_inap.no_rawat "+
                     "inner join kamar on kamar_inap.kd_kamar=kamar.kd_kamar "+
                     "where reg_periksa.status_lanjut='Ranap' "+
                     "and reg_periksa.tgl_registrasi between ? and ? "+
                     "and kamar.kd_bangsal=? "+
                     "and reg_periksa.kd_pj like ? "+
                     "and kamar_inap.stts_pulang like '%Pindah%'");
                try {
                    ps2.setString(1,Valid.SetTgl(Tgl1.getSelectedItem()+""));
                    ps2.setString(2,Valid.SetTgl(Tgl2.getSelectedItem()+""));
                    ps2.setString(3,rs.getString(1));
                    ...
                    rs2=ps2.executeQuery();
                    if(rs2.next()){
                        jmlpindah=rs2.getInt("jmlpindah");
                        ttljmlpindah+=jmlpindah;
                    }
                } finally{ if(rs2!=null) rs2.close(); if(ps2!=null) ps2.close(); }
                    
                    if(jmlpasien>0){
                        tabMode.addRow(new Object[]{
                            i,rs.getString(2),jmlpasien,jmlhari,jmlpindah
                        });
                        i++;
                    }                        
                }
                if(ttljmlhari>0){
                    tabMode.addRow(new Object[]{
                        "","Jumlah",ttljmlpasien,ttljmlhari,ttljmlpindah
                    });
                    i++;
                }
            } catch (Exception e) {
