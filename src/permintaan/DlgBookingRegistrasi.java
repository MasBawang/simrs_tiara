...
//        tampil();
    for (i = 0; i < tbObat.getRowCount(); i++) { 
        if (tbObat.getValueAt(i, 0).toString().equals("true") && tbObat.getValueAt(i, 23).toString().equals("Belum")) {
            String noRkmMedis = tbObat.getValueAt(i, 3).toString();
            String tglRegistrasi = tbObat.getValueAt(i, 5).toString();

            // Pengecekan apakah nomor RM sudah ada di tabel reg_periksa pada tanggal yang sama
            boolean sudahTerdaftar = Sequel.cariIsiAngka(
                "select count(*) from reg_periksa where no_rkm_medis = '" + noRkmMedis + 
                "' and tgl_registrasi = '" + tglRegistrasi + "'") > 0;
            
            if (!sudahTerdaftar) { // hanya melanjutkan jika belum terdaftar
                Sequel.mengedit("pasien", "no_rkm_medis = ?", 
                    "umur = CONCAT(CONCAT(CONCAT(TIMESTAMPDIFF(YEAR, tgl_lahir, CURDATE()), ' Th '), " + 
                    ...
                    1, new String[]{noRkmMedis});

                String status = Sequel.cariIsi(
                    "select if((select count(no_rkm_medis) from reg_periksa where no_rkm_medis = '" + 
                    noRkmMedis + "' and kd_poli = '" + tbObat.getValueAt(i, 8).toString() + 
                    "') > 0, 'Lama', 'Baru')");
                
                String no_rawat = Valid.autoNomer3(
                    "select ifnull(MAX(CONVERT(RIGHT(no_rawat, 6), signed)), 0) from reg_periksa where tgl_registrasi = '" + 
                    tglRegistrasi + "' ", tglRegistrasi.replace("-", "/") + "/", 6); 
                ...
                if (Sequel.menyimpantf2("reg_periksa", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "No.Rawat", 19, new String[]{
                    tbObat.getValueAt(i, 10).toString(), no_rawat, tglRegistrasi, jam(),
                    tbObat.getValueAt(i, 6).toString(), noRkmMedis, tbObat.getValueAt(i, 8).toString(),
                    tbObat.getValueAt(i, 11).toString(), tbObat.getValueAt(i, 12).toString() + ", " + tbObat.getValueAt(i, 13).toString() +
                    ", " + tbObat.getValueAt(i, 14).toString() + ", " + tbObat.getValueAt(i, 15).toString() +
                    ", " + tbObat.getValueAt(i, 16).toString(), tbObat.getValueAt(i, 17).toString(),
                    "" + Sequel.cariIsiAngka("select poliklinik.registrasilama from poliklinik where poliklinik.kd_poli = '" +
                    tbObat.getValueAt(i, 8).toString() + "'"),
                    "Belum", "Lama", "Ralan", tbObat.getValueAt(i, 18).toString(), umur, sttsumur, "Belum Bayar", status
                })) {
                    Sequel.mengedit3("skdp_bpjs", "no_rkm_medis = ? and tanggal_datang = ?", "status = 'Sudah Periksa'", 2, new String[]{
                        noRkmMedis, tglRegistrasi
                    });
                 ...
                 JOptionPane.showMessageDialog(null,"Maaf, Pasien sudah terdaftar di hari yang sama, cek registrasi");
            }
        }
    }       
        tampil();
